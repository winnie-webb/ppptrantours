...
export async function generateMetadata({ params }) {
  ...
  return {
    title: t.metaTitle ?? "Contact & FAQ",
    description: t.metaDescription,
    alternates: {
      canonical: localePath(locale, "/contact-us"),
...
function channels(t) {
  return [
    {
      Icon: FaWhatsapp,
      label: t.whatsapp ?? "WhatsApp / Messenger",
    ...
    {
      Icon: FaPhoneAlt,
      label: t.call ?? "Call us",
      value: site.contact.phone,
      href: site.contact.phoneHref,
      note: t.callNote ?? "Dispatch, 6am - 10pm daily",
    },
    {
      Icon: FaEnvelope,
      label: t.email ?? "Email",
      ...
      note: t.emailNote ?? "Replies within a few hours",
    },
  ];
}
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description={
          t.description ??
          "Send your flight number, hotel and group size and we'll come back with a plan and a firm price — usually within the hour."
        }
        image="/ppp/banner-sunset.jpg"
        locale={locale}
        homeLabel={dict.nav?.home ?? "Home"}
        breadcrumbs={[{ label: t.crumb ?? "Contact" }]}
      />
      ...
      <section className="shell py-16 lg:py-24">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr] lg:gap-16">
          <div>
            <SectionHeading
              eyebrow={t.sendEyebrow ?? "Send a message"}
              title={t.sendTitle ?? "Plan your day with us."}
              description={
                t.sendDescription ??
                "No payment is taken here — we confirm availability and price first."
              }
            />
            <ContactForm dict={client} />
          </div>

          <aside className="space-y-6">
            <div className="card p-7">
              <h3 className="font-display text-xl font-semibold text-ink">
                {t.whereTitle ?? "Where to find us"}
              </h3>
            ...
            <div className="rounded-2xl bg-ink p-7 text-white">
              <h3 className="font-display text-xl font-semibold">
                {t.transferTitle ?? "Booking an airport transfer?"}
              </h3>
              <p className="mt-2.5 text-sm leading-relaxed text-white/65">
                {t.transferBody ??
                  "Have your flight number, landing time and hotel name ready. With those three things we can confirm on the spot — and we track the flight, so a delay costs you nothing."}
              </p>
              <a
                href={site.contact.whatsappHref}
                target="_blank"
      ...
      <section className="bg-sand py-16 lg:py-24">
        <div className="shell">
          <SectionHeading
            align="center"
            eyebrow={t.faqEyebrow ?? "FAQ"}
            title={t.faqTitle ?? "Everything else, answered."}
          />
